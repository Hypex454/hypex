document.addEventListener('DOMContentLoaded', () => {
  ...
  function editProduct(product) {
    ...
    const isActiveSelect = productForm.querySelector('[name=is_active]');
    if (isActiveSelect) {
      isActiveSelect.value = product.is_active !== undefined ? product.is_active.toString() : 'true';
    }
    ...
    if (product.pc_images && product.pc_images.length > 0) {
      ...
      if (pcPreviewSection) {
        ...
        product.pc_images.forEach((img, index) => {
          pcPreviewSection.innerHTML += `
            <div style="position: relative; display: inline-block; margin-right: 0.5rem;">
              <img src="${img}" alt="Preview PC" style="width: 100px; height: 100px; object-fit: cover; border-radius: 4px; margin-bottom: 0.25rem;">
              <button type="button" class="remove-image-btn" data-product-id="${product.id}" data-image-url="${img}" data-device="pc" style="position: absolute; top: 0; right: 0; background: #dc3545; color: white; border: none; border-radius: 50%; width: 20px; height: 20px; cursor: pointer;">×</button>
            </div>
          `;
        });
    ...
    if (product.mobile_images && product.mobile_images.length > 0) {
      ...
      if (mobilePreviewSection) {
        ...
        product.mobile_images.forEach((img, index) => {
          mobilePreviewSection.innerHTML += `
            <div style="position: relative; display: inline-block; margin-right: 0.5rem;">
              <img src="${img}" alt="Preview Mobile" style="width: 100px; height: 100px; object-fit: cover; border-radius: 4px; margin-bottom: 0.25rem;">
              <button type="button" class="remove-image-btn" data-product-id="${product.id}" data-image-url="${img}" data-device="mobile" style="position: absolute; top: 0; right: 0; background: #dc3545; color: white; border: none; border-radius: 50%; width: 20px; height: 20px; cursor: pointer;">×</button>
            </div>
          `;
        });
    ...
    // Adicionar event listeners para os botões de remover imagem
    setTimeout(() => {
      document.querySelectorAll('.remove-image-btn').forEach(btn => {
        btn.addEventListener('click', async (e) => {
          ...
          try {
            const res = await fetch(`/api/products/${productId}/remove-image`, {
              method: 'POST',
              headers: { 
                'Content-Type': 'application/json',
                Authorization: `Bearer ${token}` 
              },
              body: JSON.stringify({ imageUrl, deviceType })
            });
            
            if (res.ok) {
              alert('Imagem removida com sucesso!');
              // Recarregar o produto para atualizar as previews
              const updatedRes = await fetch(`/api/products/${productId}`);
              const updatedData = await updatedRes.json();
              if (updatedData.product) {
                editProduct(updatedData.product);
              }
            } else {
